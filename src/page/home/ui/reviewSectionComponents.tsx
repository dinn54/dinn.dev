...
export const ReviewModalButton = ({
	...
	return (
		<div
			className={`flex pc:hidden w-full h-auto justify-center items-center pointer-events-auto duration-400 ease-in-out ${reviewModalOpen ? "opacity-0" : "opacity-100"}`}
		>
			<LongButton
				color="blue"
				rounded="rounded-p24"
			...
			{visible ? (
				<div className="flex w-full h-full px-[30px] py-[20px] pc:px-0 pc:py-4 pc:gap-4 justify-between items-center pc:justify-start rounded-[20px] flex-col ">
					...
					<input
						type="text"
						placeholder="이름"
						className="w-full h-p38 px-5 py-[5px] pc:h-[56px] rounded-[10px] text-p14 pc:text-p18 bg-util-input-light dark:bg-util-input-dark shrink-0 text-util text-util-input-text font-bold"
					/>
					<input
						type="text"
						placeholder="이메일"
						className="w-full h-p38 px-5 py-[5px] pc:h-[56px] rounded-[10px] text-p14 pc:text-p18 bg-util-input-light dark:bg-util-input-dark shrink-0 text-util-input-text font-bold"
					/>
					<textarea
						placeholder="메세지"
						className="w-full h-[113px] px-5 py-[14px] pc:h-[169px] rounded-[10px] text-p14 pc:text-p18 bg-util-input-light dark:bg-util-input-dark shrink-0 text-util-input-text font-bold"
					/>
					...
					<BaseButton
						color="none"
						rounded="rounded-p24"
						className="mt-2 border-inside-none pc:hidden"
						onClick={() => {
							setReviewModalOpen(false);
						}}
					>
