...
const Review = () => {
	...
	return (
		<PageContainer
			id="contact-section"
			className="bg-[#f4f6ff] dark:bg-review-dark"
		>
			<div className="flex flex-col w-full h-full justify-center items-center">
				...
				<div className="mt-[5.5rem] tab:mt-[5.75rem] pc:mt-[8rem] flex w-full z-[100] h-[calc(100%-6rem)] tab:h-[calc(100%-6.25rem)] pc:h-[calc(100%-8.5rem)] tab:px-[8.8%] pc:px-[10%] tab:py-2 shrink-0 justify-center">
					<div className="absolute flex left-0 top-[9rem] pc:top-[13rem] w-[100vw] h-[calc(100%-9rem)] pc:h-[calc(100%-13rem)] px-2 gap-10 shrink-0 flex-col pc:flex-row tab:px-[10%] pc:px-[6%] ">
						<div className="mt-3 tab:mt-10 pc:mt-5 flex w-full h-fit ">
							<div
								...
								className="relative flex w-4 h-[24rem] tab:h-[clamp(20rem,60vh,40rem)] bg-util-scrollbar-gray-light dark:bg-util-scrollbar-gray-dark "
							>
								<div
									ref={scrollMoveBarRef}
									className="absolute flex w-full h-[3rem] bg-util-scrollbar-blue-light dark:bg-util-scrollbar-blue-dark "
								/>
							</div>
							<div className="flex flex-col w-full max-h-[24rem] tab:max-h-[clamp(20rem,60vh,40rem)] overflow-y-hidden pointer-events-none">
								<div
									ref={scrollTargetRef}
									className="flex flex-col w-full h-fit gap-4 px-4 py-3 overflow-y-auto scrollbar-hide pointer-events-auto"
								>
									{reviews.map((review) => (
					...
					<div
						className={`absolute pc:hidden bottom-0 left-0 flex w-full tab:w-[70%] h-[28rem] tab:mx-[15%] z-[1]  bg-white dark:bg-util-container-bg-dark rounded-[20px] ${reviewModalOpen ? "animate-slide-up" : "animate-slide-down"} `}
					>
						<ReviewModal
							reviewModalOpen={reviewModalOpen}
							setReviewModalOpen={setReviewModalOpen}
